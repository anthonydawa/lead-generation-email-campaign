"""Run one restart-safe direct SMTP campaign worker."""
...
# The worker stays active between scheduled emails.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run one saved direct-mail campaign.")
    ...
    try:
        ...
        worker = CsvCampaignWorker(
            ...
            retry_seconds=settings.failed_send_retry_seconds,
            reply_checker=(
                ReplyChecker(settings) if settings.check_replies_before_send else None
            ),
            send_gate=SendGate(
                STATE_FOLDER,
        ...
        logging.exception("Worker configuration failed")
        raise
    print(f"Campaign ID: {campaign_id}")
    print(f"Campaign state folder: {store.directory}")
    ...
    try:
        with single_instance_lock(store.directory / "worker.lock"):
            ...
            try:
                ...
                logging.exception("Campaign worker failed")
                raise
            finally:
